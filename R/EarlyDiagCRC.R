...
#' @examples EarlyDiag_AUC(Tumor,Normal,ROC=TRUE)
#' @examples EarlyDiag(data1)

#' @export

EarlyDiag_AUC<-function(Tumor,Normal,ROC= TRUE){
  data(model)
  ...
  if(length(intersect(rownames(Tumor),makers))==2){
    ...
    print(svm_roc1$auc)
    test_result<-list(model=model1,pred1=pred1,decision.values=pred2,probability=pred3,test_data=test,svm_roc1=svm_roc1)
    if (ROC) {
